...
The flake exposes a basic module test harness through `checks.home-manager-module`.
It evaluates the Home Manager module against a generated test wallpaper and
verifies that:

- `colorScheme` and `colourScheme` resolve to the same generated data
- Kitty integration emits configuration when enabled
...
If you only want the module harness, build it directly:
